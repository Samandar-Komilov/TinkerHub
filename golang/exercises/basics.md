...
1. Create a custom error type with additional context fields.  
2. Write a function that returns an error if input is negative.  
3. Use `errors.New` to create a simple error message.  
4. Wrap an error with `fmt.Errorf` and `%w`.  
5. Unwrap a nested error and check its original type.  
6. Use `errors.Is` to check for a specific error type.  
7. Compare two errors using `errors.Is`.  
8. Handle multiple error conditions in a function.  
9. Write a function that returns `(result, error)` and handle the error.  
10. Use `defer` to ensure cleanup even if an error occurs.  
11. Create a function that panics on invalid input.  
12. Recover from a panic in a deferred function.  
13. Write a function that returns an error if a file doesn’t exist.  
14. Use `os.IsNotExist` to check for file-not-found errors.  
15. Chain multiple operations and return the first error encountered.  
16. Log an error with stack trace using `log.Printf`.  
17. Create a retry mechanism for an error-prone function.  
18. Use `errors.As` to extract a specific error type.  
19. Write a function that ignores certain errors.  
20. Simulate a network error and handle it gracefully.  
21. Create a custom error that implements `Unwrap() error`.  
22. Use `panic` for unrecoverable errors in a CLI tool.  
23. Write a function that recovers from panics in goroutines.  
24. Compare `panic` vs. returning errors for API design.  
25. Use `runtime.Stack` to print a full stack trace.  
26. Handle context cancellation as an error case.  
27. Write a function that aggregates multiple errors.  
28. Use `errors.Join` to combine multiple errors.  
29. Benchmark error creation vs. panic overhead.  
30. Implement a custom error logger with structured fields.  
