...
def reserve_room(student_id):
    ...
    with room_mutex:
        ...
        cv.wait_for(
            lambda: waiting_queue.queue[0] == student_id and available_rooms > 0
        )

        waiting_queue.get()
        available_rooms -= 1
        print(
            f"Student {student_id} reserved a room. Rooms available: {available_rooms}"
        )
    ...
    with room_mutex:
        available_rooms += 1
        print(
            f"Student {student_id} released a room in {study_time} seconds. Rooms available: {available_rooms}"
        )
        cv.notify_all()
